...
/**
 * 用户服务客户端接口（Bearer认证-Basic方式）
 * 使用@RpcClient注解标记，使用Bearer Token进行身份认证
 * <p>
 * Bearer认证方式：通过远程认证服务器获取token，在请求头中添加 Authorization: Bearer token
 * 此客户端使用Basic Auth方式获取token（client_id/client_secret通过Basic Auth传递）
 *
 * @author chuck
 */
@RpcClient(value = "user-center-bearer-basic")
public interface UserClientBearerBasic extends UserClient {
